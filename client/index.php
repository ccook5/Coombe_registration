...
else if ($action == "sign_in_out")
{
    ...
    {
        echo('<div class="notice">Scan failed, please retry.<br />Invalid scan length.</div>'."\n");
        page_footer();
    }
    else if($badge_id[0] != "¬" && $badge_id[0] != "$" && $badge_id[0] != "~" && ord($badge_id[0]) != 194)
    {
        echo('<div class="notice">Scan failed, please retry.<br />Invalid First character: '.ord($badge_id[0]).' (ascii value)</div>'."\n");
        page_footer();
    }
    else
    {
