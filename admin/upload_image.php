...
?>
  <form>
   <input type="hidden" name="action" value="add_person_handler" />
...
<?php	  
	...
	if ($result_rg)
	{
        ...
        {
			echo '<option name="r_group" value="'.$row_rg["id"].'" >'.$row_rg["name"].'</option>';
		}
	}
?>
	  </select>
    ...
    <tr>
	 ...
	 <td>
	  <select name="in_out">
	   ...
	   <option name="out" value="out" selected>Out</option>
	  </select>
	 </td>
	</tr>
    <tr>
...
?>
  <form>
   <input type="hidden" name="action" value="edit_person_handler" />
   <input type="hidden" name="old_id" value="<?php echo $person_id; ?>" />
   <table class="form_table">
    <tr>
     <td>First Name:</td>
     <td>
